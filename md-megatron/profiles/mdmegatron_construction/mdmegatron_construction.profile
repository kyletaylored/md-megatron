 ...
 */
function mdmegatron_construction_form_install_configure_form_alter(&$form, FormStateInterface $form_state) {
  $form['site_information']['site_name']['#default_value'] = 'MD Megatron Construction';
  $form['#submit'][] = 'mdmegatron_construction_form_install_configure_submit';
}

/**
 * Submission handler to sync the contact.form.feedback recipient.
 */
function mdmegatron_construction_form_install_configure_submit($form, FormStateInterface $form_state) {
  global $user;
  $sql_file = dirname(__FILE__) . '/mdmegatron_construction.sql';
  $count = mdmegatron_construction_import_sql($sql_file);  
...
function mdmegatron_construction_import_sql($filename) {
  $databasesName = array(
    "pgsql" => "PostgreSQL",
    "sqlite" => "SQLite"
  );
  ...
  if ($file) {
    ...
    if ($connect) {
      while (!feof($file)) {
        ...
        if (preg_match('|;$|', $line)) {
          try {
            $connect->exec(mdmegatron_construction_prefixTables($sql_command));
...
function mdmegatron_construction_prefixTables($sql) {
  $db = \Drupal::database();
  $connectOption = $db->getConnectionOptions();
